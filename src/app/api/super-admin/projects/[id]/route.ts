// filename: src/app/api/super-admin/projects/[id]/route.ts
...
// 🔒 SUPERADMIN-ONLY — Update project
export async function PUT(
  ...
  try {
    ...
    const existing = db
      .prepare(
        "SELECT id FROM projects WHERE (slug = ? OR title = ?) AND id != ?"
      )
      .get(slug, title, id);

    if (existing) {
      return NextResponse.json(
        { error: "Another project with this title or slug already exists." },
        { status: 400 }
      );
    }
    ...
    if (order_number !== null) {
      const existsOrder = db
        .prepare(
          "SELECT id FROM projects WHERE order_number = ? AND id != ? LIMIT 1"
        )
        .get(order_number, id);
      if (existsOrder) {
        return NextResponse.json(
...
// 🔒 SUPERADMIN-ONLY — Delete project
