...
export default function CaseStudiesDetailPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/user/case-studies/${slug}`)
      ...
      .then((data) => {
        if (data.success) {
          setCaseStudy(data.case_studies);
          setViewCount(data.case_studies.view_count || 0);
        }
      })
  ...
  const handleShare = (platform?: string) => {
    ...
    const shareText = `${caseStudy.title} - ${caseStudy.excerpt}`;
  ...
  const readingTime = Math.ceil(caseStudy?.content_html?.length / 1000) || 5;

  if (loading) return <Loading message="Loading case study..." />;
      ...
      <div className="container mx-auto px-4 max-w-5xl animate-fade-in">
        ...
        <header className="py-12 text-center">
          ...
          {caseStudy.categories?.length > 0 && (
            <div className="flex flex-wrap justify-center gap-2 mb-6">
              {caseStudy.categories.map((cat: string, index: number) => (
                <span
                  key={index}
        ...
        <div className="max-w-3xl mx-auto mt-16 pt-12 border-t border-base-200">
          ...
          <div className="grid gap-6">
            {/* Sample related case studies - you can fetch these from your API */}
