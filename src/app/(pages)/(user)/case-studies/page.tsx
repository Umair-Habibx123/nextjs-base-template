...
export default function CaseStudiesPage() {
  ...
  const [featuredCaseStudies, setFeaturedCaseStudies] = useState<CaseStudies[]>([]);
      ...
      .then((data) => {
        if (data.success) {
          ...
          setFeaturedCaseStudies(
            data.case_studies.filter((b: CaseStudies) => b.is_featured).slice(0, 3)
          );
        } else console.error("Failed to load case studies");
      })
  ...
  const filteredCaseStudies = CaseStudies
    .filter((case_studies) => {
      // Search term filter
      const matchesSearch = 
        case_studies.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
        case_studies.excerpt?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        case_studies.author_name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        case_studies.tags?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        case_studies.categories?.toLowerCase().includes(searchTerm.toLowerCase());

      // Featured filter
      const matchesFeatured = !filters.featured || 
        (filters.featured === "featured" && case_studies.is_featured === 1) ||
        (filters.featured === "not-featured" && case_studies.is_featured === 0);

      // Time range filter
      const case_studiesDate = new Date(case_studies.createdAt);
      const now = new Date();
      const matchesTimeRange = 
        !filters.timeRange ||
        (filters.timeRange === "week" && (now.getTime() - case_studiesDate.getTime()) < 7 * 24 * 60 * 60 * 1000) ||
        (filters.timeRange === "month" && (now.getTime() - case_studiesDate.getTime()) < 30 * 24 * 60 * 60 * 1000) ||
        (filters.timeRange === "year" && (now.getTime() - case_studiesDate.getTime()) < 365 * 24 * 60 * 60 * 1000);

      // View count filter
      const matchesViewCount = 
        !filters.viewCount ||
        (filters.viewCount === "popular" && case_studies.view_count > 100) ||
        (filters.viewCount === "trending" && case_studies.view_count > 50) ||
        (filters.viewCount === "new" && case_studies.view_count < 10);
  ...
  const allTags = CaseStudies.flatMap(case_studies => case_studies.tags ? case_studies.tags.split(',') : []).filter(Boolean);
  const uniqueTags = [...new Set(allTags)].slice(0, 10); // Limit to 10 tags
  const allCategories = CaseStudies.flatMap(case_studies => case_studies.categories ? case_studies.categories.split(',') : []).filter(Boolean);
  const uniqueCategories = [...new Set(allCategories)].slice(0, 8); // Limit to 8 categories

  if (loading) return <Loading message={t("Fetching Case Studies...")} />;
  ...
  return (
    <main className="min-h-screen bg-linear-to-br from-base-100 via-base-100 to-primary/5 py-8">
      ...
      <section className="container mx-auto px-4 py-8 animate-fade-in relative z-10">
        ...
        <div className="text-center mb-16 space-y-8">
          ...
          <div className="space-y-4">
            <h1 className="text-5xl md:text-6xl font-bold bg-linear-to-r from-base-content to-base-content/70 bg-clip-text text-transparent leading-tight">
              {isSearching ? t("Search Results") : t("Our Case Studies")}
            </h1>
            <p className="text-base-content/70 text-xl max-w-3xl mx-auto leading-relaxed">
              {isSearching
                ? t(`Found ${filteredCaseStudies.length} results for "${searchTerm}"`)
                : t(
          ...
          <div className="grid grid-cols-2 md:grid-cols-6 gap-4 max-w-6xl mx-auto">
            ...
            <div className="bg-linear-to-br from-base-100 to-base-200 border border-base-300/30 rounded-2xl p-4 text-center shadow-lg backdrop-blur-lg">
              <div className="text-2xl font-bold text-secondary mb-1">
                {CaseStudies.filter((case_studies) => case_studies.is_featured).length}
              </div>
              <div className="text-base-content/60 text-xs font-medium">
                {t("Featured")}
              </div>
            ...
            <div className="bg-linear-to-br from-base-100 to-base-200 border border-base-300/30 rounded-2xl p-4 text-center shadow-lg backdrop-blur-lg">
              <div className="text-2xl font-bold text-accent mb-1">
                {CaseStudies.reduce((sum, case_studies) => sum + (case_studies.view_count || 0), 0)}
              </div>
              <div className="text-base-content/60 text-xs font-medium">
                {t("Total Views")}
              </div>
          ...
          <div className="max-w-4xl mx-auto space-y-4">
            ...
            <div className="relative group">
              ...
              <input
                type="text"
                placeholder={t("Search Case Studies by title, content, author, tags...")}
                value={searchTerm}
        ...
        {/* ⭐ Featured CaseStudies Section - Only show when not searching */}
        {!isSearching && featuredCaseStudies.length > 0 && (
          <div className="mb-16">
            ...
            <div className="grid lg:grid-cols-3 gap-8">
              {featuredCaseStudies.map((case_studies, index) => (
                <article
                  key={case_studies.id}
                ...
                >
                  ...
                  {/* Image */}
                  {case_studies.cover_image && (
                    <figure className="relative h-48 overflow-hidden">
                      <img
                        src={case_studies.cover_image}
                        alt={case_studies.title}
                    ...
                    <div className="flex items-center justify-between pt-4">
                      <div className="flex items-center gap-4 text-xs text-base-content/50">
                        <div className="flex items-center gap-1">
                          ...
                          <span>
                            {new Date(case_studies.createdAt).toLocaleDateString()}
                          </span>
                        </div>
                        <div className="flex items-center gap-1">
                          <Eye className="w-4 h-4" />
                          <span>{case_studies.view_count || 0} views</span>
                        </div>
                      </div>
                      <Link
                        href={`/case-studies/${case_studies.slug}`}
        ...
        {/* 📝 case_studies Content - Dynamic Views */}
            ...
            <div className="space-y-4">
              <h3 className="text-2xl font-semibold text-base-content/60">
                {isSearching || hasActiveFilters
                  ? t("No Case Studies found matching your criteria.")
                  : t("No Case Studies published yet.")}
              </h3>
          ...
          <div className="space-y-8">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-base-content/60">
                    {isSearching
                      ? `Found ${filteredCaseStudies.length} results for "${searchTerm}"`
                      : `Showing ${filteredCaseStudies.length} of ${CaseStudies.length} articles`}
                  </p>
                </div>
              </div>
            ...
            {viewMode === "grid" && (
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                {filteredCaseStudies.map((case_studies) => (
                  <CaseStudiesGridCard key={case_studies.id} case_studies={case_studies} isSearching={isSearching} />
                ))}
              </div>
            )}
            ...
            {viewMode === "list" && (
              <div className="space-y-6">
                {filteredCaseStudies.map((case_studies) => (
                  <CaseStudiesListCard key={case_studies.id} case_studies={case_studies} isSearching={isSearching} />
                ))}
              </div>
            )}
              ...
              <div className="bg-base-100 border border-base-300/30 rounded-2xl shadow-lg overflow-hidden">
                <div className="overflow-x-auto">
                  <table className="table w-full">
                    ...
                    <tbody>
                      {filteredCaseStudies.map((case_studies) => (
                        <CaseStudiesTableRow key={case_studies.id} case_studies={case_studies} />
                      ))}
                    </tbody>
                  </table>
                </div>
...
const CaseStudiesGridCard = ({ case_studies, isSearching }: { case_studies: CaseStudies; isSearching: boolean }) => (
  <article className="group relative bg-linear-to-br from-base-100 to-base-200 border border-base-300/30 rounded-3xl shadow-lg hover:shadow-2xl transition-all duration-500 hover:scale-105 overflow-hidden">
    {case_studies.is_featured && !isSearching && (
    ...
    <div className="p-6 space-y-4">
      ...
      <div className="flex items-center gap-4 text-xs text-base-content/50 pt-2">
        <div className="flex items-center gap-1">
          <Calendar className="w-4 h-4" />
          <span>{new Date(case_studies.createdAt).toLocaleDateString()}</span>
        </div>
        <div className="flex items-center gap-1">
          <Eye className="w-4 h-4" />
          <span>{case_studies.view_count || 0} views</span>
...
const CaseStudiesListCard = ({ case_studies, isSearching }: { case_studies: CaseStudies; isSearching: boolean }) => (
    ...
    <div className="flex flex-col md:flex-row">
      ...
      <div className="flex-1 p-6 space-y-3">
        <div className="flex items-start justify-between gap-4">
          ...
          <Link
            href={`/case-studies/${case_studies.slug}`}
            className="btn btn-primary btn-sm rounded-xl flex-shrink-0 group/btn transition-all duration-300 transform hover:scale-105"
          >
            <ArrowRight className="w-4 h-4 group-hover/btn:translate-x-1 transition-transform duration-300" />
          </Link>
        </div>
        <div className="flex items-center gap-4 text-xs text-base-content/50">
          <div className="flex items-center gap-1">
            ...
            <span>{case_studies.author_name}</span>
          </div>
          <div className="flex items-center gap-1">
...
const CaseStudiesTableRow = ({ case_studies }: { case_studies: CaseStudies }) => (
  <tr className="hover:bg-base-200/50 transition-colors duration-200 group">
    ...
    <td className="py-4">
      <div className="text-sm text-base-content/80">{case_studies.author_name}</div>
    </td>
    <td className="py-4">
      <div className="flex items-center gap-1 text-sm">
        <Eye className="w-4 h-4 text-base-content/60" />
        {case_studies.view_count || 0}
      </div>
    </td>
    <td className="py-4">
      <div className="flex items-center gap-2">
        {case_studies.is_featured && (
          <Star className="w-4 h-4 text-warning fill-current" />
        )}
        <span className={`badge badge-sm capitalize ${
          case_studies.is_featured ? 'badge-warning' : 'badge-outline'
        }`}>
          {case_studies.is_featured ? 'Featured' : 'Standard'}
        </span>
      </div>
    </td>
    <td className="py-4">
