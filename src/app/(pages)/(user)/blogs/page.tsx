    ...
    fetch(`/api/blogs?page=${page}&limit=12`)
      ...
      .then((data) => {
        if (data.success) {
          ...
          setFeaturedBlogs(
            data.blogs.filter((b: Blog) => b.is_featured).slice(0, 3)
          );
        } else console.error("Failed to load blogs");
      })
        ...
        <div className="text-center mb-16 space-y-8">
          ...
          <div className="space-y-4">
            <h1 className="text-5xl md:text-6xl font-bold bg-linear-to-r from-base-content to-base-content/70 bg-clip-text text-transparent leading-tight">
              {isSearching ? t("Search Results") : t("Our Blog")}
            </h1>
            <p className="text-base-content/70 text-xl max-w-3xl mx-auto leading-relaxed">
              {isSearching
                ? t(`Found ${filteredBlogs.length} results for "${searchTerm}"`)
                : t(
            ...
            <div className="space-y-4">
              <h3 className="text-2xl font-semibold text-base-content/60">
                {isSearching || hasActiveFilters
                  ? t("No blogs found matching your criteria.")
                  : t("No blogs published yet.")}
              </h3>
...
const BlogTableRow = ({ blog }: { blog: Blog }) => (
  <tr className="hover:bg-base-200/50 transition-colors duration-200 group">
    ...
    <td className="py-4">
      <div className="text-sm text-base-content/80">{blog.author_name}</div>
    </td>
    <td className="py-4">
