...
import BlogForm from "../components/blogs-management/BlogForm";
...
export default function BlogManagerPage() {
  ...
  const featuredCount = blogs.filter((b) => b.is_featured === 1).length;
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/super-admin/blogs/${deleteId}`, {
        method: "DELETE",
      });
      if (res.ok) {
  ...
  const uniqueAuthors = [
    ...new Set(blogs.map((blog) => blog.author_name).filter(Boolean)),
  ];
  const allTags = blogs
  ...
  const filteredBlogs = blogs.filter((blog) => {
    ...
    // Author filter
    const matchesAuthor =
      !filters.author || blog.author_name === filters.author;
    ...
    const matchesDateRange =
      ...
      (!filters.dateRange.end || blogDate <= new Date(filters.dateRange.end));
      ...
      <div className="flex items-center gap-4 p-6 bg-linear-to-r from-primary/5 via-secondary/5 to-accent/5 rounded-3xl border border-base-300/20 backdrop-blur-lg">
        ...
        <div className="hidden lg:flex items-center gap-2 px-4 py-2 bg-primary/10 rounded-full border border-primary/20 backdrop-blur-sm">
          ...
          <span className="text-sm font-medium text-primary">
            {statsLoading ? "Loading..." : `${stats.totalPosts} Posts`}
          </span>
        </div>
        {!editingBlog && (
          <button
        ...
        <div className="bg-linear-to-br from-base-100 to-base-200 border border-base-300/30 rounded-2xl shadow-2xl backdrop-blur-lg p-6 transition-all duration-500">
          <div className="flex flex-col lg:flex-row justify-between items-center gap-4 mb-4">
            <div className="flex items-center gap-3 flex-1 max-w-md">
              <input
                type="text"
                placeholder="Search blogs by title, excerpt, author, tags..."
                value={searchTerm}
                      ...
                      <td className="py-4 border-r border-base-300/20">
                        <div className="max-w-xs">
                          ...
                          <div className="flex items-center gap-2 mt-1">
                            ...
                            {blog.order_number !== null && (
                              <div className="text-xs text-primary font-medium">
                                Order: {blog.order_number}
                              </div>
                            )}
                          </div>
                        </div>
